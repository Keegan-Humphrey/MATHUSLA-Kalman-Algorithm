...
void VertexFinder_c_b::Seed_k_m()
{
...
void VertexFinder_c_b::FindVertices_k_m_hybrid()
{
	if (seeds_k_m.size() < 1)
		return; // no seeds
...
void VertexFinder_c_b::FindVertices_k()
{
	if (seeds_k_m.size() < 1)
		return;
...
//			if (std::abs(kfv.pulls_v[k]) < kalman_c_b::pull_cut_drop) {

			double beta = (kfv.added_tracks[k]->q_s).norm() / constants::c;

//			if (!(kalman_c_b::v_cut_drop[0] < beta && beta < kalman_c_b::v_cut_drop[1])) {
			if (!(par_handler->par_map["v_cut_drop[0]"] < beta && beta < par_handler->par_map["v_cut_drop[1]"])) {
//			if (!(kalman_c_b::v_cut_drop[0] < kfv.pulls_v[k] && kfv.pulls_v[k] < kalman_c_b::v_cut_drop[1])) {

...
/*
	for (int first = 0; first < hits.size(); first++)
	{
		...
		{
*/
	for (int first = 0; first < hits_c_b.size(); first++)
	{
		for (int second = first + 1; second < hits_c_b.size(); second++)
		{

			int layer1 = ((hits_c_b[first])->det_id).layerIndex;
			int layer2 = ((hits_c_b[second])->det_id).layerIndex;

			if (layer1 == layer2)
				continue;

			double ds_2 = c_score(hits_c_b[first], hits_c_b[second]);
			...
			//seeds.push_back(seed(hits[first], hits[second]));
			seeds_c_b.push_back(seed(hits_c_b[first], hits_c_b[second]));
...
void TrackFinder::MergeTracks_c_b()
{
	//at this point, all of the points have been fit to tracks. At this point, we will perform the track merging step.

	if (tracks_c_b.size() == 0 or tracks_c_b.size() == 1)
		return;
	...
	for (int first_track = 0; first_track < tracks_c_b.size(); first_track++)
	{

		for (int second_track = first_track + 1; second_track < tracks_c_b.size(); second_track++)
		{
			auto tr1 = tracks_c_b[first_track];
			auto tr2 = tracks_c_b[second_track];
...
//			if (distance < cuts::merge_distance and cos_theta > cuts::merge_cos_theta){
	...
	for (int k = 0; k < tracks_c_b.size(); k++)
	{
		...
		if (add)
		{
			good_tracks.push_back(tracks_c_b[k]);
		}
	}

	tracks_c_b = good_tracks;

	//at this point, the list of tracks is finalized. Now they will be indexed:
	for (int trackn=0; trackn<tracks_c_b.size(); trackn++)
	{
		tracks_c_b[trackn]->index = trackn;
	}
	//file.close();
}
...
void TrackFinder::CalculateMissingHits_c_b(Geometry *geo)
{
	//calculate for kalman tracks
	for (auto track : tracks_c_b)
	{
		std::vector<int> layers = track->layers();
		std::vector<int> expected_layers;
...
void TrackFinder::FindTracks_c_b()
{

	if (par_handler->par_map["debug"] == 1) std::cout << "Number of seeds:" <<  seeds_c_b.size() << std::endl;

	if (seeds_c_b.size() == 0)
		return; //no seeds found in initial seeding, will be retried with <c travel

	int index = 0;
	int total_hits = hits_c_b.size();
	bool iterate = true;
	int j = 0;
	int MAX_ITS = 25;
	...
	while (iterate)
	{
		...
		int min_index = min_seed_c_b();
		auto current_seed = seeds_c_b[min_index];

		seeds_c_b.erase(seeds_c_b.begin() + min_index); //delete the seed so that it isn't used again
		...
		kf_find.kalman_all(hits_c_b, &current_seed);
...
//		if (current_track->nlayers() >= cuts::track_nlayers && chi_sum < cuts::kalman_track_chi)
		if (current_track->nlayers() >= cuts::track_nlayers && chi_sum < par_handler->par_map["kalman_track_chi"])
		{
			tracks_c_b.push_back(current_track);
		...
		hits_c_b = unused_hits;

		if (seeds_c_b.size() == 0)
			iterate = false;
		if (hits_c_b.size() < cuts::nseed_hits)
			iterate = false;

	}
	// assign indices
	for (int i=0; i < tracks_c_b.size(); i++)
        {
                tracks_c_b[i]->index = i;
        }
